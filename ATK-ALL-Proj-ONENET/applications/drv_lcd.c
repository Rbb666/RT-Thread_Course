...
void LCD_Color_Fill(uint16_t sx, uint16_t sy, uint16_t ex, uint16_t ey, uint16_t *color)
{
    ...
    for(i = 0; i < height; i++)
    {
        ...
        for(j = 0; j < width; j++)LCD->RAM = color[i * width + j]; //写入数据
    }
}
...
static int drv_fmc_init(void)
{
    ...
    res = rt_memheap_init(&sramheap, "sram", (void*) Bank1_SRAM_ADDR, EXT_SRAM_SIZE);

    if (res == RT_EOK)
    {
        rt_kprintf("sram init ok!\n");
    }

    return RT_EOK;
}

INIT_DEVICE_EXPORT(drv_fmc_init);

struct rt_device_graphic_ops fsmc_lcd_ops = { LCD_Fast_DrawPoint, LCD_ReadPoint, LCD_HLine,
           RT_NULL, LCD_BlitLine,
};
...
static rt_err_t drv_lcd_control(struct rt_device *device, int cmd, void *args)
{
    ...
    switch (cmd)
    {
        case RTGRAPHIC_CTRL_GET_INFO:
        {
            ...
            info->width = SSD_HOR_RESOLUTION;
            info->height = SSD_VER_RESOLUTION;
        }
        break;
    }
